    ...
    <Space direction="vertical" :style="{ width: '100%' }" size="large">
      <Card>
        ...
        <div class="seat-wrapper">
          <div v-for="(item,index) in state.currentSeat" :key="index">
<!--            <span>{{-->
...
<!--              （ <span class="price">￥{{ item?.price }}</span
            >）-->
<!--            <span>{{ item.quantity >= 1 ? item.quantity+'张票' : '1张票' }}</span>-->
              <span>{{'剩余'+item.quantity+'张票' }}</span>
              </span>
          </div>
        </div>
        <div class="tip">
          *剩余座位供您参考，具体座位以您确认订单时实际的座位为准。
        </div>
      </Card>
      <Card>
        <template #title>
      ...
      <div class="tips-txt">
        ...
        <p>
          1.
          一张有效证件同一乘车日期同一车次只能预约一个座位。
        </p>
        <p>
          2.一月内5次申请座位成功后取消订单，当月将不能在校车在线预约系统继续购票。
        </p>
        <p>3.未尽事宜详见《操作手册》等有关规定和通知公告。</p>
      </div>
    </Space>
    <Modal
      :visible="state.open"
    ...
    >
      <Space direction="vertical" :style="{ width: '100%' }">
        ...
        >
          <a href=""
            >*如果本次校车剩余席位无法满足您的选座需求，系统将自动为您分配席位</a
          >
